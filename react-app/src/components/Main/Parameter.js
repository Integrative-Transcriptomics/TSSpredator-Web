...
function Parameter({ parameter, onChange, grid }) {
  ...
  if (!isNaN(parameter.value)) {
    ...
    return (
      <div className={grid ? 'parameter-grid' : 'parameter-box margin-left'} data-title={parameter.tooltip}>
  ...
  // combobox
    ...
    return (
      <div className={grid ? 'parameter-select' : 'parameter-box margin-left'} data-title={parameter.tooltip}>
        <label className='element'> {parameter.name}</label>
